...
func NewServerStatus(mongoService *services.MongoServerService) *ServerStatus {
	return &ServerStatus{
...
func (s *ServerStatus) Export() (string, error) {
	...
	servers, _, err := s.mongoService.GetAll(services.Query{})
	if err != nil {
		return "", err
	}
...
func (s *ServerStatus) Check(id string) (bool, error) {
	server, err := s.mongoService.GetById(id)
	if err != nil {
	...
	if err != nil {
		if strings.Contains(err.Error(), "ssh") && strings.Contains(err.Error(), "handshake") {
			server.Status = true
			_, _ = s.mongoService.Update(id, server)
			return true, nil
		}
		server.Status = false
		_, _ = s.mongoService.Update(id, server)
